...
import { parsedStats } from "./scripts/fetch.js";
...
const showInfo = async () => {
  ...
  roster.forEach((player) => {
    if (player.id % 2 === 0) {
      ...
      const sumOfPoints =
        stats[player.id - 1].stats.goals + stats[player.id - 1].stats.assists;
      ...
      const averageTimePerGame =
        stats[player.id - 1].stats.iceTime / stats[player.id - 1].stats.games;
      timeOnIce.classList.add("text-white");
      timeOnIce.innerText = `Avg minutes per game: ${averageTimePerGame.toFixed(
        2
      )}`;
